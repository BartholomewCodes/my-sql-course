...
WHERE ptl.PropertyTypeCode NOT IN (
    SELECT DISTINCT pp.PropertyType FROM PricePaidSW12 pp WHERE pp.Street = 'Ranmere Street'
);
...
FROM
    PricePaidSW12 pp
WHERE
    pp.Street = 'Ranmere Street' AND 
    pp.Price > (SELECT AVG(pp.Price) FROM PricePaidSW12 pp WHERE pp.Street = 'Ranmere Street')
ORDER BY pp.Price ;
